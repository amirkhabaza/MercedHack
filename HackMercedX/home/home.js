...
let apiKey = 'O3RCEW4DJXHIF3L7'; // Your Alpha Vantage API key

// Predefined list of stocks with their beta values (from your file)
const predefinedStockDatabase = [
...
// Dynamic stock database (from your friend's file)
let stockDatabase = [];
...
// Function to populate the stock database with predefined and dynamic data
async function populateStockDatabase() {
  // Start with predefined stocks
  stockDatabase = [...predefinedStockDatabase];
  ...
  const symbolsToProcess = stockDatabase.slice(0, 5); // Start with 5 stocks

  for (const stock of symbolsToProcess) {
    try {
      // Fetch both quote and overview data
      const [quoteData, overviewData] = await Promise.all([
        fetchStockQuote(stock.symbol),
        fetchStockOverview(stock.symbol)
      ]);
      
      if (quoteData && overviewData) {
        // Update stock data with real-time information
        stock.price = parseFloat(quoteData.price);
        stock.beta = parseFloat(overviewData.Beta || stock.beta);
        stock.riskScore = calculateRiskScore(stock.beta);
      }
    } catch (error) {
...
// Function to update popular stocks by risk category
